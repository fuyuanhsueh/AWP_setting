...
/** 頂部導航列 — Electron 環境用按鈕觸發 dialog，瀏覽器環境用 file input */
  ...
  return (
    <header className="h-16 px-6 flex items-center justify-between bg-bg-card border-b border-border-main sticky top-0 z-40">
      <div className="flex items-center gap-3">
        ...
        <p className="text-[10px] text-text-dim font-mono uppercase tracking-widest">系統版本 v4.2.0 // {isElectron ? 'Electron 桌面版' : '生產環境'}</p>
      </div>
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* JSON 匯入：Electron 用按鈕，瀏覽器用 file input */}
          {isElectron ? (
            <button onClick={() => onImportJson()} title="從 JSON 匯入進度" className="p-2 bg-slate-800 hover:bg-slate-700 border border-border-main rounded-md text-text-dim hover:text-blue-400 transition-all">
          ...
          <div className="h-6 w-[1px] bg-border-main mx-1" />
          {/* YAML 匯入：Electron 用按鈕，瀏覽器用 file input */}
          {isElectron ? (
            <button onClick={() => onImportYaml()} title="匯入 YAML 配置" className="p-2 bg-slate-800 hover:bg-slate-700 border border-border-main rounded-md text-text-dim hover:text-emerald-400 transition-all">
